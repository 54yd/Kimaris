...
const App = () => {
	...
	const [_invert_strength, setFilterInvertParam] = useState(
		FIRST_FILTER_INVERT_PARAM
	)
	const [_saturate_strength, setFilterSaturateParam] = useState(
		FIRST_FILTER_SATURATE_PARAM
	)
	const [_brightness_strength, setFilterBrightnessParam] = useState(
		FIRST_FILTER_BRIGHTNESS_PARAM
	)
	...
	const [playBGMending, { stop: stopBGMending }] = useSound(BGMending, {
		playbackRate,
		volume: volumeEnding + volumeWeight,
		loop: true,
	})
	...
	const [playSFXcar03, { stop: stopSFXcar03 }] = useSound(SFXcar03, {
		volume: 0.3,
	})
	const [playSFXcar04, { stop: stopSFXcar04 }] = useSound(SFXcar04)
	const [playSFXcar05, { stop: stopSFXcar05 }] = useSound(SFXcar05)
	const [playSFXCarCrush, { stop: stopSFXCarCrush }] = useSound(
		SFXCarCrush,
		{
			volume: 1.0 - 0.3,
		} /* [MEMO] due to BGM pitch craziness event make feel certainly, isnormalize slightly */
	)
	...
	const [playSFXscan03Alt, { stop: stopSFXscan03Alt }] = useSound(SFXscan03, {
		volume: 0.35,
	})

	const [playSFXtakedown02, { stop: stopSFXtakedown02 }] =
		useSound(SFXtakedown02)
	const [playSFXtakedown00, { stop: stopSFXtakedown00 }] =
		useSound(SFXtakedown00)
	const [playSFXYellAndATK, { stop: stopSFXYellAndATK }] =
		useSound(SFXYellAndATK)
	const [playSFXLevelUp, { stop: stopSFXLevelUp }] = useSound(SFXLevelUp)

	const [playSFXrobot01, { stop: stopSFXrobot01 }] = useSound(SFXrobot01, {
		volume: 0.5,
	})
	const [playSFXrobot02, { stop: stopSFXrobot02 }] = useSound(SFXrobot02, {
		volume: 0.5,
	})
	const [playSFXrobot06, { stop: stopSFXrobot06 }] = useSound(SFXrobot06, {
		volume: 0.35,
	})

	const [playSFXretro02, { stop: stopSFXretro02 }] = useSound(SFXretro02, {
		volume: 0.3,
	})
	const [playSFXretro10, { stop: stopSFXretro10 }] = useSound(SFXretro10, {
		volume: 0.3,
	})
	const [playSFXbeep05, { stop: stopSFXbeep05 }] = useSound(SFXbeep05, {
		volume: 0.2,
	})
	...
	const [currentHPBattleEvent, setCurrentHPBattleEvent] = useState(
		FULL_HPBATTLE_EVENT_NUM
	)
			...
			'camera-orbit':
				currStage == 1
					? Number(HPEnemy) * 36 +
					  ...
					  Number(HPEnemy) * 36 +
					  'deg ' +
					  Math.abs(Math.tan(Number(HPEnemy) * Math.PI)) * 3.0 +
					  'm'
		...
		rat001: {
			...
			9960: {
				mes:
					...
					signal +
					(deviceKind == 'iPhone'
						? ' OSバージョンは' + userDataDetail.OS
						: '') +
					'の通信を使ってるんだね。…………ああ、そっか、スマホってみんな好き嫌いあると思うし、こういう話題って、あんまり弾まないのかな。………人となんて話したことなかったから、そこの「あなた」と、普通のお話がしてみたかったの。………ただ、それだけの気持ちなの。」',
				snd: playSFXscan03,
			},
			9950: {
				mes:
					'「' +
					browser +
					'からこちらを見ているのね。私もそれ好き。他にも色々な種類があると思うんだけど、あなたは詳しい方なのかしら？そういうのを見てるとね、たまに、すっごく不思議なモノを使ってる人もいたりするの。……人って、本当に、習慣や好みで分かれるものよね。」',
				snd: playSFXscan03,
			},
			9940: {
				mes:
					'「IPアドレス、' +
					userDataDetail.IP +
					'って所に住んでるんだね。ねえ……？………今から、そっちに遊びに行ってもいい？　ようし。…………………………。　　・・・・・・・・・。ちょっと待っててね、今すぐ行くから……。……………。・・・・・・・。………………なんてね。私には画面の向こう側に行く力なんてないの。ビックリさせちゃった？…あなたのことを沢山知りたくて、頑張ってみたけれど、私ができるのは、ただこうして画面を通して、あなたと話をすることぐらい…。」',
				snd: playSFXscan03,
			},
			9930: {
	...
	useEffect(() => {
		...
		if (currBGMIndex == 1) {
			playBGMDelegate = playBGMending
			stopBGMDelegate = stopBGMending
		}
	...
	useEffect(() => {}, [playbackRate])

	useEffect(() => {
		if (window.innerWidth > PLAYABLE_MAX_WIDTH) {
			setIsOverWidth(true)
		} else {
	...
	useEffect(() => {
		...
		if (sBrowser == 'unknown' || sBrowser == null) {
			setBrowser('XXXXX')
		} else {
			setBrowser(sBrowser)
		}
		...
		let phonename =
			(md.phone()?.toUpperCase() == 'UNKNOWNPHONE' ? 'UNKNOWNPHONE' : false) ||
			md.mobile()
		if (phonename != null && phonename?.toUpperCase() == 'UNKNOWNPHONE')
			phonename = false
	...
	*/
	...
	const logicBattle = async ({ event }) => {
		...
		if (HPEnemy > 0) {
			...
			let damageParam =
				player.ATK +
				parseInt(
					Math.random() * 10 /*///[TEMPORARY COMMENT OUT]///Math.random()*10*/
				) -
				5 -
				enemyList?.['E00' + (currStage + 1)]?.DEF

			console.log(
				'STAGE:' + currStage + ' ' + ' DAMAGE:' + damageParam + ' HP:' + HPEnemy
			)
			if (damageParam <= 0 || isGuard == true) {
				damageParam = MIN_DAMAGE_PARAM
			}
			...
			for (const property in mesBattle?.[
				enemyList?.['E00' + (currStage + 1)]?.id
			]) {
			...
			console.log(
				'playbackRate:' +
					(5.0 * 1.0) /
						(1.0 - HPEnemy / enemyList?.['E00' + (currStage + 1 + 1)]?.HP) +
					1
			)
			//hell bgm
			...
			if (currStage == MAX_STAGE_COUNT - 1 - 1 && HPEnemy <= 20 + 20) {
				setPlaybackRate(
					//  1 / (1-(MAX/x=normX)) +1 <- by adding +1 can move pivot right then soften smoothness between 0 to 1 in x axis
					(5.0 * 1.0) /
						(1.0 - HPEnemy / enemyList?.['E00' + (currStage + 1 + 1)]?.HP) +
						1
				)
			...
			//  SPELL GUARD IS ON
			if (
				HPEnemy <= enemyList?.['E00' + (currStage + 1)]?.GuardThreshold &&
				isGuard == false
			) {
			...
			if (currStage < MAX_STAGE_COUNT - 1) {
				...
				console.log(
					'E00' +
						(currStage + 1) +
						' ' +
						enemyList?.['E00' + (currStage + 1 + 1)]?.HP
				)
	...
	const backdropFilterHellGeneral = css`
		backdrop-filter: contrast(${_contrast_strength}) invert(${_invert_strength})
			blur(${_blur_strength}) grayscale(${_grayscale_strength})
			brightness(${_brightness_strength}) hue-rotate(${_hue_strength})
			sepia(${_sepia_strength}) saturate(${_saturate_strength});
	`
	...
	const showModal = (key) => (options) => {
		...
		//OnShowing
		if (key == 'modalStageClear') {
			playSFXLevelUp()
			console.log(key)
		}
		if (key == 'modalTakedown') {
			playSFXtakedown00()
			console.log(key)
		}
	}
		...
		<div className={RootDesign}>
			...
			<a.div
				...
				<div
					...
					className={backdropFilterHellGeneral}></div>
			</a.div>
			...
			<div
				style={{
					...
					userDrag:
						'none' /*Webpack compile this to be compatible with Webkit*/,
					userSelect:
						'none' /*Webpack compile this to be compatible with Webkit*/,
					WebkitTouchCallout:
						'none' /* disable the IOS popup when long-press on a link */,
			...
			<div className={frameRootDesign}>
				...
				<div
					...
					<div
						style={{
							marginLeft: '-50%',
							marginTop: screenSize.height / (2 - 0.1 + 0.4),
							transform:
								'translateX(16.5%) perspective(10cm) rotate3d(1, 0, 0, 0.15turn)',
							display: 'flex',
							justifyContent: 'center',
							alignItems: 'center',
				...
				<div
					...
					<div
						style={{
							...
							marginTop: '30%',
							transform:
								'translateX(16.5%) perspective(10cm) rotate3d(1, 0, 0, -0.05turn)',
							display: 'flex',
							justifyContent: 'center',
							alignItems: 'center',
							fontSize: screenSize.height / (3 + 3 - 2 - 1) + 'px',
...
*/}
			...
			{isOverWidth || (
				<NoticeBar
					...
					marqueeProps={{
						...
						leading: 1000,
						trailing: 5000,
						style: { padding: '0 7.5px', fontFamily: 'PixelMPlus' },
					}}
					style={{
						marginTop: 10 + 16 + 3 + 'px',
			...
			<button
				...
				onClick={
					currStage < MAX_STAGE_COUNT - 1 - 1
						? toggleBGM
						: null /*///[TEMPORARY COMMENT OUT]///toggleBGM*/
				}>
				{isBGMPlaying ? (
			...
			{document.querySelector('button#ar-button-1') && (
				<button
					onClick={() => {
						document.querySelector('button#ar-button-1').click()
						playSFXclick8()
						console.log(
							document.querySelector('button#ar-button-1') +
								'ar button 1 clicked'
			...
			{isOverWidth ? (
				<Card
					style={{
						margin: 'auto',
						padding: '20px',
						top: `${screenSize.height / (1.0 + 1.0 - 0.7)}px`,
						blur: '20px',
					...
					className={backdropFilter1}>
					<strong>🍊 SCRIBBLE 🎍🏠🎍 お知らせ 🍊</strong>
					<WhiteSpace sm />
					スマホの専用機能を使っているため、ゲームモードを起動できません。
					<WhiteSpace lg />
					スマホからお楽しみくだされ　
					<WhiteSpace lg />
					<strong> ( ただしアヒルとはふれあえます )</strong>
				</Card>
			) : (
				<Parallax
					...
					className={
						parallaxRootDesign
					} /* config: This config, retrict the slippy move to zero Then Cut Spring Physics for Perdormance */
					config={{ duration: 0 }}>
					<ParallaxLayer
						offset={1}
						speed={0}
						className={unTouchable}
						style={{ backgroundColor: '#0aceff', zIndex: '0' }}
					/>
					<ParallaxLayer
						offset={2}
						speed={0}
						className={unTouchable}
						style={{ backgroundColor: '#448ef6', zIndex: '0' }}
					/>
					<ParallaxLayer
						offset={3}
						speed={0}
						className={unTouchable}
						style={{ backgroundColor: '#65daf7', zIndex: '0' }}
					/>
					<ParallaxLayer
						offset={4}
						speed={0}
						className={unTouchable}
						style={{ backgroundColor: '#81e1af', zIndex: '0' }}
					/>
					<ParallaxLayer
						offset={5}
						speed={0}
						className={unTouchable}
						style={{ backgroundColor: '#012B67', zIndex: '0' }}
					/>
					<ParallaxLayer
						offset={6}
						speed={0}
					...
					<ParallaxLayer offset={0.3} speed={0} style={{ zIndex: '0' }}>
						...
						<div className={ArticleRootDesign}>
							...
							<div className={ArticleContainer}>
								...
								{[...Array(/*WhiteSpaceRepeatCount*/ 21)].map((_, i) => (
									<WhiteSpace lg />
								))}
								<a.div
									style={{
										opacity: fadeMusicButtonX.to({
											range: [0, 1],
											output: [0.05, 1],
										}),
									...
									<Button
										...
										style={{
											...
											color: 'rgba(255,255,255,1)', //"rgba(255,255,255,0.3)",
											display: 'flex',
											justifyContent: 'center',
											alignItems: 'center',
											fontFamily: 'Poppins',
								...
								<WingBlank>
									...
									<Modal
										...
										afterClose={() => {
											...
											//else playSFXerror14()
											mesBattle?.[enemyList?.['E00' + (currStage + 1)]?.id]?.[
												currentHPBattleEvent
											]?.snd?.()
										...
										<div style={{ height: '100px', overflow: 'scroll' }}>
											...
											<Progress
												percent={parseInt(
													(HPEnemy / enemyList?.['E00' + (currStage + 1)]?.HP) *
														100
												)}
												position="normal"
												unfilled={true}
											...
											/>
											{HPEnemy}/{/*20*/}19
											{enemyList?.['E00' + (currStage + 1)]?.assignedYearSuffix}
										</div>
									</Modal>
									...
									<Modal
										...
										<div style={{ height: '100px', overflow: 'scroll' }}>
											...
											<Progress
												...
												unfilled={true}
												barStyle={{ borderRadius: '5px' }}
												style={{}} /*appearTransition*/
											/>
											{HPEnemy}/20
											{enemyList?.['E00' + (currStage + 1)]?.assignedYearSuffix}
											<br />《{enemyList?.['E00' + (currStage + 1)]?.name}
											》は特殊な魔法陣を発動した・・・！！！
										</div>
									</Modal>
									...
									<Modal
										...
										title={
											'空 想 切 除 - ' +
											enemyList?.['E00' + currStage]?.iconicKanji
										}
										footer={[
											{
												text: 'GO NEXT',
												onPress: (e) => {
													onClose('modalStageClear')(e)
												},
											},
										]}
										wrapProps={{ onTouchStart: onWrapTouchStart }}
									...
									<Modal
										...
										<div
											...
											<div style={{ fontSize: '20px' }}>
												{enemyList?.['E00' + (currStage + 1 - 1)]?.name}
											</div>
										</div>
									</Modal>
								</WingBlank>
...
*/}
					...
					<ParallaxLayer
						...
						style={{
							...
							alignItems: 'center',
							justifyContent: 'center',
							/*gap:20+15+"px", [FIXME] cannot be applied by iOS Safari... so I set margin below into the children */ zIndex:
								'100',
						}}>
						<img
							src={server1}
						...
						<img
							src={server3}
							className={touchable}
							style={{ margin: '10px 10px', width: '20%', top: 30 + 'px' }}
							onClick={(e) => {
								e.preventDefault
								playSFXrobot06()
							}}
					...
					{false ? ( ///[TEMPORARY COMMENT OUT]///(currStage<MAX_STAGE_COUNT-3)
						...
						<ParallaxLayer
							...
							<img
								...
								src={GameTown}
								style={{
									transform: 'translateX(-40%) translateY(-28%) scale(0.7)',
								}}
							/>
							<img
								onClick={(e) => {
								...
								src={rat}
								style={{
									transform: 'translateX(80%) translateY(-2828%) scale(0.2)',
									zIndex: 100 + 1,
								}}
							/>{' '}
							{/*20, 2228*/}
							<img
								...
								src={rat}
								style={{
									transform: 'translateX(30%) translateY(-2228%) scale(0.5)',
									zIndex: 100 + 1,
								}}
							/>{' '}
							{/*20, 2228*/}
